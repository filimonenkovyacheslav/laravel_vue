...
class Good extends \App\Http\Models\BaseModel
{
    use Sluggable;
    ...
    public static $langTable = 'good_langs';
    public static $langKey = 'good_id';
    public static $langFields = 'good_langs';
    public static $defaultImage = 'post-logo.jpg';
    
    
    public static function getGoodData($key = '') {
        $data = [
            'status' => [
    ...
    public static function _canGoodEdit($entity, $adminOnly = false) {
        $user = Auth::user();
        $admin = $user->isAdmin();
    ...
    public static function getAll($params = [], $with = [], $orderBy = 'id', $order = 'asc')
    {
        ...
        if (!isset($params['category'])) {
            $categories = GoodCategory::getAllListParent();
            if (!empty($categories)) {
                $categories = array_keys($categories);
                $categories = implode(',', $categories);
                $entities = $entities->whereRaw("{$prefix}id IN (SELECT good_id FROM good_category_relation WHERE good_category_id IN ({$categories}))");
            }
        }
        ...
        $entities->with($with);
        if ($defaultOrder) {
            $entities->orderBy('goods.label', 'asc')->orderBy('position', 'asc')->orderBy($orderBy, $order);
        } else {
            $entities->orderBy($orderBy, $order)->orderBy('goods.label', 'asc')->orderBy('position', 'asc');
        }
    ...
    public static function saveItem($request, $preset = false, $langsData = []) {
        ...
        if(!$preset && !$isAdmin && $data['author'] != $user->id) {
            return ['message' => __('This is not your good.'), 'errors' => []];
        }
        ...
        if(!$preset && !$new && !static::_canGoodEdit($entity)) {
            return redirect(url('/'));
        }
        ...
        if(isset($langsData) && sizeof($langsData) > 0) {
            foreach($langsData as $langId => $langData) {
                $lang = GoodLang::where([[self::$langKey, $id], ['lang_id', $langId]])->first();
                if(!$lang) {
        ...
        $entity['categories'] = GoodCategory::saveGoodCategories($id, $categories);
    ...
    {
        $entity = static::find($id);
        if(!static::_canGoodEdit($entity, false)) {
            return redirect(url('/'));
        }
        $user = Auth::user();
        if ($user->isAdmin()) {
            ...
            GoodCategory::deleteGoodCategories($id);
        } else {
            static::setGoodStatus($id, 5);
        }
        return true;
    }
    
    public static function setGoodStatus($id, $status)
    {
        $entity = static::find($id);
        if(!static::_canGoodEdit($entity, $status == 6)) {
            return redirect(url('/'));
        }
        if(!static::_canGoodEdit($entity, $status == 1)) {
            return redirect(url('/'));
        }
    ...
    public static function setGoodLabel($id, $label)
    {
        $entity = static::find($id);
        
        if(!static::_canGoodEdit($entity, true)) {
            return redirect(url('/'));
        }
        
        $labels = static::getGoodData('label');
    ...
    public static function _afterGet($entity, $role = null, $relation = null)
    {
        ...
        if(!empty($entityData)) {
            ...
            $id = $entityData['id'];
            $entityData['is_favorite'] = GoodFavorite::isGoodFeatured($id);
            ...
            $entityData['fields'] = GoodCategory::_getGoodFieldsList();
            $entityData['categories'] = GoodCategory::getGoodCategories($id);
            $entityData = static::_replaceLangFields($entityData);
            $goodData = static::getGoodData();
            foreach($goodData as $k => $d) {
                $entityData[$k . '_view'] = [];
            ...
            $entityData = [
                ...
                'fields' => GoodCategory::_getGoodFieldsList(),
                'categories' => [],
                'user' => [],
                'keywords' => []
            ];
    ...
    public static function searchGoodForLocations($keyword, $limit = 15)
    {
    ...
    public static function getGoodFilters( $params = [] )
    {
        return [];
    }
}
