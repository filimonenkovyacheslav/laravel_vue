...
class Product extends \App\Http\Models\BaseModel
{
    use Sluggable;
    ...
    public static $langTable = 'product_langs';
    public static $langKey = 'product_id';
    public static $langFields = 'product_langs';
    public static $defaultImage = 'post-logo.jpg';
    
    
    public static function getProductData($key = '') {
        $data = [
            'status' => [
    ...
    public static function _canProductEdit($entity, $adminOnly = false) {
        $user = Auth::user();
        $admin = $user->isAdmin();
    ...
    public static function getAll($params = [], $with = [], $orderBy = 'id', $order = 'asc')
    {
        ...
        if (!isset($params['category'])) {
            $categories = ProductCategory::getAllListParent();
            if (!empty($categories)) {
                $categories = array_keys($categories);
                $categories = implode(',', $categories);
                $entities = $entities->whereRaw("{$prefix}id IN (SELECT product_id FROM product_category_relation WHERE product_category_id IN ({$categories}))");
            }
        }
        ...
        $entities->with($with);
        if ($defaultOrder) {
            $entities->orderBy('products.label', 'asc')->orderBy('position', 'asc')->orderBy($orderBy, $order);
        } else {
            $entities->orderBy($orderBy, $order)->orderBy('products.label', 'asc')->orderBy('position', 'asc');
        }
        //dd($entities->toSql());
    ...
    public static function saveItem($request, $preset = false, $langsData = []) {
        ...
        if(!$preset && !$isAdmin && $data['author'] != $user->id) {
            return ['message' => __('This is not your product.'), 'errors' => []];
        }
        ...
        if(!$preset && !$new && !static::_canProductEdit($entity)) {
            return redirect(url('/'));
        }
        if (is_null($data['price']) || empty($data['price'])) $data['price_default'] = 0;
        ...
        if(isset($langsData) && sizeof($langsData) > 0) {
            foreach($langsData as $langId => $langData) {
                $lang = ProductLang::where([[self::$langKey, $id], ['lang_id', $langId]])->first();
                if(!$lang) {
        ...
        $entity['categories'] = ProductCategory::saveProductCategories($id, $categories);
    ...
    {
        $entity = static::find($id);
        if(!static::_canProductEdit($entity, false)) {
            return redirect(url('/'));
        }
        $user = Auth::user();
        if ($user->isAdmin()) {
            ...
            ProductCategory::deleteProductCategories($id);
        } else {
            static::setProductStatus($id, 5);
        }
        return true;
    }
    
    public static function setProductStatus($id, $status)
    {
        $entity = static::find($id);
        if(!static::_canProductEdit($entity, $status == 6)) {
            return redirect(url('/'));
        }
        if(!static::_canProductEdit($entity, $status == 1)) {
            return redirect(url('/'));
        }
    ...
    public static function setProductLabel($id, $label)
    {
        $entity = static::find($id);
        
        if(!static::_canProductEdit($entity, true)) {
            return redirect(url('/'));
        }
        
        $labels = static::getProductData('label');
    ...
    public static function _afterGet($entity, $role = null, $relation = null)
    {
        ...
        if(!empty($entityData)) {
            ...
            $id = $entityData['id'];
            $entityData['is_favorite'] = ProductFavorite::isProductFeatured($id);
            ...
            $entityData['fields'] = ProductCategory::_getProductFieldsList();
            $entityData['categories'] = ProductCategory::getProductCategories($id);
            $entityData = static::_replaceLangFields($entityData);
            $productData = static::getProductData();
            foreach($productData as $k => $d) {
                $entityData[$k . '_view'] = [];
            ...
            $entityData = [
                ...
                'fields' => ProductCategory::_getProductFieldsList(),
                'categories' => [],
                'user' => [],
                'keywords' => []
            ];
    ...
    public static function searchProductForLocations($keyword, $limit = 15)
    {
    ...
    public static function getProductFilters( $params = [] )
    {
        return [];
    }
}
