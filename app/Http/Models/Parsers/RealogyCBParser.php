<?php
    ...
    class RealogyCBParser extends BaseParser
    {
        public static $parserParams = ['skip'];
        public static $userName = '';
        public static $userEmail = 'info@medicaleer';
        public static $imageName = 'medicaleer_realogycb_';
        ...
        public static function run(Parser $parser)
        {
            ...
            static::prepareTempDir('realogycb_temp');
                ...
                for($k = $params['skip']; $k <= $end; $k += $limit) {
                    ...
                    if($parsedResult['done'] == 0) {
                        ...
                        if(is_null($parsedResult['url'])) {
                            $link = $url.'/listings/?sortBy=listedOn%20asc&select=listingId&brandCodes='.$brand.'&top='.$limit;
                    ...
                    if($blockNum >= static::$maxBlocks) {
                        Setting::setValue('parsers', 'realogycb_skip', $k);
                        static::$properties = [];
        ...
        public static function parse($response, $info, $request)
        {
            ...
            switch($reqType) {
                case 'list':
                    ...
                    if (!isset($responseJson['results']) || empty($responseJson['results'])) {
