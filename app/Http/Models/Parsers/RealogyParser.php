<?php
    ...
    class RealogyParser extends BaseParser
    {
        public static $parserParams = ['skip'];
        public static $userName = '';
        public static $userEmail = 'info@medicaleer';
        public static $imageName = 'medicaleer_realogy_';
        public static $parseLimit = 0;
        public static $parseBlockLimit = 100;
        ...
        public static $iso2 = '';
        public static $brands = ['COR']; //'','','','' //'BHG','CBC','ERA','SIR','COR','C21'
...
//        private static $keyApi = 'kJB7gnrSmSfsRPMfa248RVduey5IntJw';
    ...
    // preprod data
        private static $tokenUrl = 'https://realogy.oktapreview.com/oauth2/ausdtpyw647fbrcPi0h7/v1/token';
        private static $scope = 'https://btt.realogyfg.com/searchapi';
        private static $grantType = 'client_credentials';
        private static $tokenApi = 'eyJraWQiOiJtd0IxOGNVNm52d016TFlYZkpSTG4xcll6QTN5R3FzdHFoajNkZ1NaYnA0IiwiYWxnIjoiUlMyNTYifQ..IKwgwhnZFWwyQEzI8iBEBdRTbiTxlVyqo2DGO3Xk_aoz8B3Nw1mgVbFYWHOQ0fJ6JwahOhCQ4MwNcJPe7BbW9-nMdxAkpmoMcqCDnP6EKzHWM0llcH217FKPcbHKmSiOfIHc8jXWjkfeRDkcqlSo9rMsUxkQKfRCjvte8nwC4r4mscf8zfbbDihfQcHrV5-eCwd4BZIj2qXdXDFMkX3b7hHC8qI1Yh4_Bxr943E8ucA-fWQ87shmur2_3YVIkPd--oWqmxYOLGFneYFYcRHYsjQweyjsLXi6BOI03Kl6l2Sj6LzVp7CFX5lN5lxymcO1xmw4mWqLaKIieJjsw7TZjw';
        private static $clientId = '0oaq900oaqi3PNFL90h7';
        private static $clientSecret = 'uXuIEHxaR-RePHGncJv-yBjSPem7rMHMssKXBTrQ';
        private static $keyApi = 'mzkqR3tgZolI3OHMEmMPYaQAtyQJVVO1';
        //end preprod data
        ...
        public static function controlTokenApi()
        {
            if ((time() + 300) > static::$tokenExpires) {
                ...
                if (!empty($data) && isset($data['access_token'])) {
                    ...
                    echo 'token :: '.(static::$tokenApi).PHP_EOL;
                } else {
        ...
        public static function run(Parser $parser)
        {
            ...
            static::prepareTempDir('realogy_temp');
            static::$imageName = 'medicaleer_realogy2_' . date('YmdHis').'_';
            static::controlTokenApi();
            
            $curl = new RollingCurl([$model . 'Parser', 'parse']);
            $curl->setReferer('https://preprod1.api.realogy.com');
...
//                            $link = $url.'/companies/?top='.$limit;
                            $link = $k ? $link.'&skip='.$k : $link;
                        } else {
                            $link = $parsedResult['url'];
                        }
                        $curl->get($link, null, null, ['type' => 'list', 'url' => $link, 'block' => $blockNum]);
//                        $curl->get($link, null, null, ['type' => 'companies', 'url' => $link, 'block' => $blockNum]);
                        echo '!!!!!'.$link.PHP_EOL;
                    }
                    if($blockNum >= static::$maxBlocks) {
                        Setting::setValue('parsers', 'realogy_skip', $k);
                        static::$properties = [];
        ...
        public static function parse($response, $info, $request)
        {
            ...
            switch($reqType) {
                case 'companies':
                    static::controlTokenApi();
                    echo 'Parse companies TIME :: '.(microtime(true) - static::$start).PHP_EOL;
                    if (!isset($responseJson['results'])) {
                        print_r($responseJson);
                        static::setError('Wrong results: '.$request->url, true,
                    ...
                    echo 'cntComps:'.count($listings).PHP_EOL;
                    if($cntProps > 0) {
                        $cntNew = 0;
                        foreach($listings as $n => $listing) {
                            if(empty($listing)) continue;
                            
                            $link = static::$url . '/offices?companyId=' . $listing['companySummary']['id'];
                            $log = ['method' => 'get', 'url' => $link, 'company_id' => $listing['companySummary']['id']];
                            if(static::isParsed($log, 2)) {
                                static::$blocks[$reqBlock]['parsed']++;
                                echo 'isParsed->'.(static::$blocks[$reqBlock]['parsed']).PHP_EOL;
                            } else {
                                $cntNew++;
                                static::$blocks[$reqBlock]['name'] = $listing['companySummary']['name'];
                                static::$blocks[$reqBlock]['companyId'] = $listing['companySummary']['id'];
                                $curl->get($link, null, null, ['type' => 'offices', 'log' => $log, 'block' => $reqBlock]);
                            }
                        }
                    }
                ...
                case 'offices':
                    ...
                    echo 'Parse offices TIME :: '.(microtime(true) - static::$start).PHP_EOL;
                    ...
                    $cntProps = intval($responseJson['count']);
                    echo 'cntOff:'.$cntProps.PHP_EOL;
                    if($cntProps > 0) {
                        $offices = $responseJson['results'];
                        $mainOffice = null;
                        foreach($offices as $n => $office) {
                            if(empty($office)) continue;
                
                            $office = $office['officeSummary'];
                            if (($cntProps > 1 && !isset($office['type'])) || ($cntProps > 1 && stripos($office['type'], 'Main office') === false)) {
                                continue;
                            } else {
                                $mainOffice = $office;
                                break;
                        ...
                        if (is_null($mainOffice)) {
                            $mainOffice = array_shift($offices);
                        }
                        
                        $companyName = isset($mainOffice['companyName']) ? $mainOffice['companyName'] : static::$blocks[$reqBlock]['name'];
                        $companyName = trim($companyName);
                        $user = static::getUser($companyName);
                        ...
                        if (!$user) {
                            ...
                            $company = [
                                ...
                                'role_id' => 3,
                                'name' => $companyName,
                                'companyId' => $mainOffice['companyId']
                            ];
                            ...
                            if ($pos === false) {
                                ...
                                $company['first_name'] = trim(substr($companyName, 0, $pos));
                                $company['last_name'] = trim(substr($companyName, $pos + 1));
                            }
    
                            if (isset($mainOffice['officeAddress'])) {
                                $company['map_address']
                                    = $company['address'] = isset($mainOffice['officeAddress']['formattedAddress'])
                                    ? $mainOffice['officeAddress']['formattedAddress'] : '';
                                $company['lat'] = isset($mainOffice['officeAddress']['latitude'])
                                    ? $mainOffice['officeAddress']['latitude'] : '';
                                $company['lng'] = isset($mainOffice['officeAddress']['longitude'])
                                    ? $mainOffice['officeAddress']['longitude'] : '';
                                $company['city'] = isset($mainOffice['officeAddress']['city'])
                                    ? $mainOffice['officeAddress']['city'] : '';
                                $len = strlen($mainOffice['officeAddress']['country']);
                                $resultCountry = [];
                                if ($len == 2) {
                                    $resultCountry = Country::where('iso2', $mainOffice['officeAddress']['country'])
                                        ->get()->toArray();
                                } elseif ($len == 3) {
                                    $resultCountry = Country::where('iso3', $mainOffice['officeAddress']['country'])
                                        ->get()->toArray();
                                } elseif ($len > 0) {
                                    //$resultCountry = Country::whereRaw('("name" ILIKE '.DB::getPdo()->quote('%'.$mainOffice['officeAddress']['country'].'%').')')
                                    $resultCountry = Country::where('name', 'ILIKE', DB::getPdo()->quote('%'.$mainOffice['officeAddress']['country'].'%'))
                                        ->get()
                                        ->toArray();
                                }
                                if ($resultCountry) {
                            ...
                            static::saveNewUser($company, $data['log'], []);
                        }
                    } else {
                        $companyName = static::$blocks[$reqBlock]['name'];
                        $companyName = trim($companyName);
                        $user = static::getUser($companyName);
                        ...
                        if (!$user) {
                            $company = [
                                ...
                                'role_id' => 3,
                                'name' => $companyName,
                                'companyId' => static::$blocks[$reqBlock]['companyId']
                            ];
                    ...
                    echo 'End Parse offices TIME :: '.(microtime(true) - static::$start).PHP_EOL;
                    break;
                case 'list':
                    static::controlTokenApi();
                    echo 'Parse list TIME :: '.(microtime(true) - static::$start).PHP_EOL;
                    ParserLog::saveUrlData(6, $request->url, 'list=> ' . $response);
                    if (!isset($responseJson['results'])) {
                    ...
                    if($cntProps > 0) {
                        ...
                        foreach($listings as $n => $listing) {
                            if(empty($listing)) continue;
                            $link = str_replace('searchlistings', 'search/listings', $listing['link']);
                            if(!$link || strlen($link) < 10) continue;
    
                            $logLink = 'https://api.realogy.com/v1.0/search/listings/' . $listing['listingSummary']['listingId'];
                            $log = ['method' => 'get', 'url' => $logLink, 'listing_id' => $listing['listingSummary']['listingId']];
                            if(static::isParsed($log, 2)) {
                                static::$blocks[$reqBlock]['parsed']++;
                                echo 'isParsed->'.(static::$blocks[$reqBlock]['parsed']).PHP_EOL;
                            } else {
                ...
                case 'property':
                    ...
                    ParserLog::saveUrlData(6, $request->url, 'property=> ' . $response);
                    ...
                    if (empty($summary)) {
                        static::setError('Empty property result: '.$request->url, false, static::getUrlParts($request->url));
                    }
                    ...
                    $data['status'] = 1; //$summary['isActive'] ? 1 : 2;
                    ...
                    if (isset($responseJson['media']) && !empty($responseJson['media'])) {
                        ...
                        if($imagesCnt > 0) {
                            ...
                            foreach($responseJson['media'] as $n => $media) {
                                ...
                                $link = str_replace('qa-integration-images-1.azurewebsites.net/api/', 'openapi.azureedge.net/', $link);
        ...
        public static function getUser($companyName) {
            $slug = trim(preg_replace("/[^A-Za-z0-9 ]/","",$companyName));
            $slug = preg_replace("/\s+/","-",$slug);
            $slug = strtolower($slug);
            $user = User::select('id')
                ->where([['users.name','LIKE',$companyName]])
                ->orWhere([['users.slug', 'LIKE', $slug]])
                ->first();
        ...
        public static function saveProperty($data, $ident, $reqBlock)
        {
            ...
            if(isset($data['images']) && sizeof($data['images']) > 0) {
                ...
                if($cnt == 0) {
                    static::$imagesErrors++;
                    static::setError('Images Load Error(2): '.$data['cnt_images'], false, $log);
                    $error = true;
                } else {
            ...
            if (!$error) {
                ...
                echo 'Saved Property: ' . (isset($log['entity_id']) ? $log['entity_id'] : '-').PHP_EOL;
