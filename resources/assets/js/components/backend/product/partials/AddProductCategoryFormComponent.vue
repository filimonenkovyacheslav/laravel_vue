<template>
    <div class="add-category-form">
        ...
        <form id="save-productCategory-form" action="/save-productCategory" method="POST" class="add-frontend-productCategory">
            <div class="submit-form-wrap">
                ...
                <button type="button" class="btn btn-primary btn-block" @click="onSubmitProductCategory()">{{ trans('Save') }}</button>
            </div>
        </form>
    </div>
</template>
...
<script>
	export default {
        ...
        methods: {
            onSubmitProductCategory: function() {
                var self = this,
                    form = $('#save-productCategory-form'),
                    formData = form.serialize();
    
                axios.post('/save-productCategory/back', formData).then(function(response) {
                    self.$parent.entity.categories.push(response.data.product_category_id);
                    form.find('#name,#slug').val('');
                    form.find('#parent_id').val(0).trigger('change');
                    self.closeModal();
                    self.$eventHub.$emit('productCategoryLoaded');
