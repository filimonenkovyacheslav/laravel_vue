<template>
    <div class="add-category-form">
        ...
        <form id="save-furnitureCategory-form" action="/save-furnitureCategory" method="POST" class="add-frontend-furnitureCategory">
            <div class="submit-form-wrap">
                ...
                <button type="button" class="btn btn-primary btn-block" @click="onSubmitFurnitureCategory()">{{ trans('Save') }}</button>
            </div>
        </form>
    </div>
</template>
...
<script>
	export default {
		name: "AddFurnitureCategoryFormComponent",
        mounted: function() {
            var self = this;
        },
        methods: {
            onSubmitFurnitureCategory: function() {
                var self = this,
                    form = $('#save-furnitureCategory-form'),
                    formData = form.serialize();
    
                axios.post('/save-furnitureCategory/back', formData).then(function(response) {
                    self.$parent.entity.categories.push(response.data.furniture_category_id);
                    form.find('#name,#slug').val('');
                    form.find('#parent_id').val(0).trigger('change');
                    self.closeModal();
                    self.$eventHub.$emit('furnitureCategoryLoaded');
