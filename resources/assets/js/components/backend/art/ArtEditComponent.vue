        ...
        <div class="container">
            <div class="row dashboard-inner-main">
                <div class="col-lg-10 col-md-9 col-sm-12 dashboard-inner-left">
                    <form id="save-art-form" action="/save-art" method="post" enctype="multipart/form-data" class="add-frontend-property">
                        <div class="submit-form-wrap">
                            <div class="account-block form-step active">
                                <div class="add-title-tab">
                                    <h3>{{ trans('Professional description and price') }}</h3>
                                        ...
                                        <div class="row">
                                            <div class="col-sm-12" v-if="params.user_role=='administrator' || params.agency_agents[params.user_role]">
                                                <div :class="['form-group', errorsList.author ? 'has-danger' : '']">
                                                    <label for="author_name">{{ trans('Professional') }} *</label>
                                                    <input type="text" name="author_name" id="author_name" v-model="entity.author_name"
                                                           :class="['form-control', errorsList.author ? 'form-control-danger' : '']" :placeholder="trans('Enter artist name or ID')" />
                                            ...
                                            <div class="col-sm-12">
                                                <div :class="['form-group', errorsList.title ? 'has-danger' : '']">
                                                    <label for="title">{{ trans('Professional Title') }} *</label>
                            ...
                            <art-edit-admin-categories></art-edit-admin-categories>
                            <art-edit-admin-images></art-edit-admin-images>
                            <art-edit-admin-location></art-edit-admin-location>
                ...
                <div class="col-lg-2 col-md-3 col-sm-12 dashboard-inner-right">
                    <div class="dashboard-sidebar">
                        <div class="dashboard-sidebar-inner">
                            ...
                            <a v-if="entity && entity.slug" :href="route('art.view.frontend', {'slug': entity.slug})" class="btn btn-default btn-block">{{ trans('Professional View') }}</a>
                            <a :href="route('user.profile.arts')" class="btn btn-default btn-block">{{ trans('Professional Categories') }}</a>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <art-edit-admin-add-category></art-edit-admin-add-category>
...
<script>
    ...
    export default {
        ...
        created: function() {
            ...
            axios.post('/api/art' + id, { _token: this.csrf }).then(function(response) {
        ...
        methods: {
            onSubmit: function() {
                ...
                tinyMCE.triggerSave();
                self.entity.description = $('#save-art-form [name="description"]').val();
                self.entity.price = $('#save-art-form input[name="price"]').val();
                self.entity.categories = $('#save-art-form input[name="categories"]').val();
                self.entity.photos = [];
                ...
                axios.post('/save-art', self.entity).then(function(response) {
