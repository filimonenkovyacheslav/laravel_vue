				...
				<div class="col-md-12">
					<div class="form-group">
						...
						<select id="bulkActions" class="form-control bulk-edit-user-statuses" style="display: inline-block; max-width:200px;">
							...
							<option v-for="item, index in params.furniture_status" :value="item.id">{{ item.label }}</option>
						</select>
                        <button class="btn btn-primary" style="margin-top:-2px" @click.stop="bulkEdit()">{{ trans('Apply') }}</button>
                        <button v-for="item, name in params.furniture_labels" class="btn btn-secondary ml-1" style="margin-top:-2px" @click.stop="bulkLabel(name)">{{ item.label }}</button> 
                        ...
                        <form action="" method="get" autocomplete="off" id="search-entity-form">
                            <div class="profile-top-left">
                                <div class="row no-margin">
                                    ...
                                    <div class="form-group no-margin" style="width:200px">
                                        <select name="furniture_labels" class="form-control bulk-edit-user-labels" style="display: inline-block; max-width:200px;" :value="params.filter.furniture_labels">
                                            <option selected disabled value="">{{ trans('Labels') }}</option>
                                            <option v-for="item, index in params.furniture_labels" :value="item.id">{{ item.label }}</option>
                                        </select>
                                    </div>
                                ...
                                <div class="media my-property">
                                    ...
                                    <div class="media-left">
                                        <div class="figure-block">
                                            <figure class="item-thumb">
                                                <a v-bind:href="route('furniture.view.frontend', {'slug': prop.slug})">
                                    ...
                                    <div class="media-body align-self-center">
                                        <div class="my-description">
                                            <h4 class="my-heading">
                                                <a v-bind:href="route('furniture.view.frontend', {'slug': prop.slug})">{{ prop.title }}</a>
                                            </h4>
                                            <div class="item-categories" v-if="prop.categories.length">
                                                <strong>Categories:</strong> <span class="item-category" v-for="category, index in prop.categories" v-if="params.furniture_categories_front[category]">{{ params.furniture_categories_front[category] }}</span>
                                            </div>
                                            <div class="status">
                                                <p>
                                                    <span>
                                        ...
                                        <div v-if="prop.author==params.user.id || inArray(params.user_role, ['administrator'])" class="my-actions">
                                            <div class="btn-group">
                                                ...
                                                <ul class="dropdown-menu actions-dropdown">
                                                    <li><a v-bind:href="route('furniture.edit.admin', {'id': prop.id})"><i class="fa fa-edit"></i> {{ trans('Edit') }}</a></li>
                                                    <li v-if="inArray(prop.status, [2, 5, 6]) && inArray(params.user_role, ['administrator'])">
                                                        <a v-bind:href="route('furniture.status.admin', {'id': prop.id, 'status': 1})">
                                                            <i class="fa fa-check"></i> {{ trans('Approve') }}
                                                        </a>
                                                    </li>
                                                    <li v-for="item, name in params.furniture_labels" v-if="prop.status==1 && prop.label!=item.id && inArray(params.user_role, ['administrator'])">
                                                        <a v-bind:href="route('furniture.label.admin', {'id': prop.id, 'label': name})" class="label-property">
                                                    ...
                                                    <li v-if="prop.status==1 && prop.label && inArray(params.user_role, ['administrator'])">
                                                        <a v-bind:href="route('furniture.label.admin', {'id': prop.id, 'label': 0})" class="label-property">
                                                            <i class="fa fa-remove"></i> {{ trans('Remove label') }}
                                                        </a>
                                                    </li>
													<li v-if="prop.status!='6' && prop.status!='5'">
                                                        <a v-bind:href="route('furniture.unpublish.admin', {'id': prop.id})" class="unpublish-property">
                                                            <i class="fa fa-minus"></i> {{ trans('Unpublish') }}
                                                        </a>
                                                    </li>
                                                    <li v-if="prop.status!='5' || (prop.status=='5' && inArray(params.user_role, ['administrator']))">
                                                        <a v-bind:href="route('furniture.delete.admin', {'id': prop.id})" class="delete-property">
                                                            <i class="fa fa-trash"></i> {{ trans('Delete') }}
                                                        </a>
                                                    </li>
                                                    <li v-if="prop.status!='5'">
                                                        <a v-bind:href="route('furniture.clone.admin', {'id': prop.id})" class="clone-property">
...
<script>
    ...
    export default {
        props: ['params'],
        mounted: function() {
            this.entities_list = this.params.user.furnitures;
            var self = this,
            ...
            self.getAddressKeywordsAutocomplete(locationInput, {type: 'furniture'},
                function(item, event) {
		...
		methods: {
			bulkEdit: function() {
				...
				axios.post('/bulk-edit-furnitures', formData).then(function(response) {
			...
			bulkDelete: function() {
				...
				formData = new FormData();
				formData.append("editItems", editItems);
				axios.post('/bulk-delete-furnitures', formData).then(function(response) {
                    self.messageData = response.data.message ? response.data.message : '';
                    self.errorsExist = response.data.errors_exist ? response.data.errors_exist : false;
                    self.errorsList = response.data.errors ? response.data.errors : [];
                    self.entity = response.data.entity ? response.data.entity : self.entity;
        ...
        bulkLabel: function(label) {
                ...
                axios.post('/bulk-label-furnitures', formData).then(function(response) {
                    self.messageData = response.data.message ? response.data.message : '';
                    self.errorsExist = response.data.errors_exist ? response.data.errors_exist : false;
                    self.errorsList = response.data.errors ? response.data.errors : [];
                    self.entity = response.data.entity ? response.data.entity : self.entity;
