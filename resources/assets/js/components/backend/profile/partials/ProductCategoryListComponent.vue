<template>
    <div class="user-dashboard-right dashboard-with-panel">
        <div class="board-header board-header-4">
            <div class="row">
                <div class="col-sm-12 col-xs-12">
                    <div class="board-header-left">
                        <h3 class="board-title">{{ trans('Product Categories') }}</h3>
                    </div>
                    <div class="board-header-right">
                        <ol class="breadcrumb">
                            <li itemscope="" itemtype="http://data-vocabulary.org/Breadcrumb">
                                <a itemprop="url" v-bind:href:href="route('home')">
                                    <span itemprop="title">{{ trans('Home') }}</span>
                                </a>
                            </li>
                            <li class="active">{{ trans('Product Categories') }}</li>
                        </ol>
                    </div>
                </div>
            </div>
        </div>
        <div class="dashboard-content-area dashboard-fix">
            <div class="row" v-if="params.user_role=='administrator'">
                <div class="col-md-12">
                    <div class="row">
                        <div class="col-md-12" v-for="counter, counterName in params.counters">
                            <div class="form-group">
                                <b>{{ counter.title }}:</b>
                                <span>(<b>{{ trans('total') }}:</b> {{ counter.count }})</span>
                                <span>(<b>{{ trans('published') }}:</b> {{ counter.published }})</span>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="col-md-12">
                    <div class="form-group">
                        <select id="bulkActions" class="form-control bulk-edit-list" style="display: inline-block; max-width:200px;">
                            <option selected disabled value="">{{ trans('Bulk Actions') }}</option>
                            <option value="publish">{{ trans('Publish') }}</option>
                            <option value="unpublish">{{ trans('Unpublish') }}</option>
                            <option value="delete">{{ trans('Delete') }}</option>
                        </select>
                        <button class="btn btn-primary" style="margin-top:-2px" @click.stop="bulkEdit()">{{ trans('Apply') }}</button>
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="my-profile-search">
                        <div class="profile-top-left">
                            <form method="get" action="">
                                <input type="hidden" name="prop_status" value="">
                                <div class="single-input-search">
                                    <input class="form-control" name="name" :placeholder="trans('Search name')" type="text" :value="params.filter.name">
                                    <button type="submit"></button>
                                </div>
                            </form>
                        </div>
                        <div class="profile-top-right">
                            <div class="sort-tab text-right">
                                <a v-bind:href="route('productCategory.edit.admin')" class="btn btn-primary">{{ trans('Add Category') }}</a>
                            </div>
                        </div>
                    </div>
                    <div class="my-property-listing">
                        <div class="row grid-row">
                            <div class="item-wrap">
                                <div class="media my-property">
                                    <input id="toggleBulkButton" type="checkbox" v-on:change="toggleBulkEditItems" style="margin-right:10px;">
                                    <div class="media-left">
                                        <div class="figure-block">
                                            <figure class="item-thumb" style="line-height:1.1;">
                                                <label for="toggleBulkButton" style="line-height:1;margin:0;">{{ trans('Select all') }}</label>
                                            </figure>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="row grid-row" v-for="item, index in entities_list.data">
                            <div class="item-wrap">
                                <div class="media my-property">
                                    <input type="checkbox" :value="item.product_category_id" class="bulkEditItems" id="bulkEditItems" style="margin-top:47px"/>
                                    <div class="media-body align-self-center">
                                        <div class="my-description">
                                            <h4>{{ item.name }}</h4>
                                            <div class="status">
                                                <p>
                                                    <span><strong>{{ trans('ID') }}:</strong> #{{ item.product_category_id }}</span>
                                                </p>
                                                <p>
                                                    <span><strong>{{ trans('Status') }}:</strong> <template v-if="item.status">{{ trans('Published') }}</template><template v-else>{{ trans('Unpublished') }}</template></span>
                                                </p>
                                                <p>
                                                    <span><strong>{{ trans('Products') }}:</strong> {{ item.total_products }}</span>
                                                </p>
                                            </div>
                                        </div>
                                        <div class="my-actions">
                                            <div class="btn-group">
                                                <button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">{{ trans('Actions') }} <i class="fa fa-angle-down"></i></button>
                                                <ul class="dropdown-menu actions-dropdown">
                                                    <li><a v-bind:href="route('productCategory.edit.admin', {'id': item.product_category_id})"><i class="fa fa-edit"></i> {{ trans('Edit') }}</a></li>
                                                    <li v-if="inArray(params.user_role, ['administrator'])">
                                                        <a v-if="!item.status" v-bind:href="route('productCategory.status.admin', {'id': item.product_category_id, 'status': 1})">
                                                           <i class="fa fa-check"></i> {{ trans('Publish') }}
                                                        </a>
                                                        <a v-else v-bind:href="route('productCategory.status.admin', {'id': item.product_category_id, 'status': 0})">
                                                            <i class="fa fa-minus"></i> {{ trans('Unpublish') }}
                                                        </a>
                                                    </li>
                                                    <li><a v-bind:href="route('productCategory.delete.admin', {'id': item.product_category_id})" class="delete-jobCategory"><i class="fa fa-close"></i> {{ trans('Delete') }}</a></li>
                                                </ul>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <template v-if="item.children">
                                <user-profile-product-category-child :entity_item="item" :params="params"></user-profile-product-category-child>
                            </template>
                        </div>
                        <hr>
                    </div>
                    <pagination :pagination="entities_list"></pagination>
                </div>
            </div>
        </div>
    </div>
</template>

<script>
    /**
     * Get data via props in blade template <vue_template :props></vue_template>
     */
    export default {
        props: ['params'],
                mounted: function() {
            this.entities_list = this.params.productCategories;
        },
        methods: {
            bulkEdit: function() {
                var editItems = new Array();
                var action = jQuery('.bulk-edit-list').val();
                jQuery('.bulkEditItems').each(function(){
                    if (jQuery(this).is(':checked')) {
                        editItems.push(jQuery(this).val());
                    }
                });
                if(editItems.length && action && action.length) {
                    var self = this,
                        formData = new FormData();

                    formData.append('editItems', editItems);
                    formData.append('action', action);
                    axios.post('/bulk-edit-prod-cats', formData).then(function(response) {
                        self.messageData = response.data.message ? response.data.message : '';
                        self.errorsExist = response.data.errors_exist ? response.data.errors_exist : false;
                        self.errorsList = response.data.errors ? response.data.errors : [];
                        self.entity = response.data.entity ? response.data.entity : self.entity;
                        if(response.data.message === 'Done') {
                            setTimeout(function() {
                                window.location.href = window.location.href;
                            }, 500);
                        }
                    }).catch(function(error) {
                        console.log(error);
                    });
                }
            },
        }
    }
</script>
