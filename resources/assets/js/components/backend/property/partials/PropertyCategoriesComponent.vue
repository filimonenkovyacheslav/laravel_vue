<template>
    <div class="account-block form-step active">
        <div class="add-title-tab">
            <h3>{{ trans('Property Categories') }}</h3>
        </div>
...
<script>
    ...
    export default {
        ...
        mounted: function() {
            ...
            this.$eventHub.$on('propertyCategoryLoaded', function() {
                self.getPropertyCategories();
            });
        ...
        methods: {
            ...
            getPropertyCategories: function() {
                var self = this;
                axios.post('/api/propertyCategories', { _token: this.csrf }).then(function(response) {
