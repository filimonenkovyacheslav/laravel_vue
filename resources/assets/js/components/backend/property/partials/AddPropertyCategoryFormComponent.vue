<template>
    <div class="add-category-form">
        ...
        <form id="save-propertyCategory-form" action="/save-propertyCategory" method="POST" class="add-frontend-propertyCategory">
            <div class="submit-form-wrap">
                ...
                <button type="button" class="btn btn-primary btn-block" @click="onSubmitPropertyCategory()">{{ trans('Save') }}</button>
            </div>
        </form>
    </div>
</template>
...
<script>
	export default {
		name: "AddProductCategoryFormComponent",
        mounted: function() {
            var self = this;
        },
        methods: {
            onSubmitPropertyCategory: function() {
                var self = this,
                    form = $('#save-propertyCategory-form'),
                    formData = form.serialize();
    
                axios.post('/save-propertyCategory/back', formData).then(function(response) {
                    self.$parent.entity.categories.push(response.data.property_category_id);
                    form.find('#name,#slug').val('');
                    form.find('#parent_id').val(0).trigger('change');
                    self.closeModal();
                    self.$eventHub.$emit('propertyCategoryLoaded');
