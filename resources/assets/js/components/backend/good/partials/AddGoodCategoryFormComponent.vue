<template>
    <div class="add-category-form">
        ...
        <form id="save-goodCategory-form" action="/save-goodCategory" method="POST" class="add-frontend-goodCategory">
            <div class="submit-form-wrap">
                ...
                <button type="button" class="btn btn-primary btn-block" @click="onSubmitGoodCategory()">{{ trans('Save') }}</button>
            </div>
        </form>
    </div>
</template>
...
<script>
	export default {
		name: "AddGoodCategoryFormComponent",
        mounted: function() {
            var self = this;
        },
        methods: {
            onSubmitGoodCategory: function() {
                var self = this,
                    form = $('#save-goodCategory-form'),
                    formData = form.serialize();
    
                axios.post('/save-goodCategory/back', formData).then(function(response) {
                    self.$parent.entity.categories.push(response.data.good_category_id);
                    form.find('#name,#slug').val('');
                    form.find('#parent_id').val(0).trigger('change');
                    self.closeModal();
                    self.$eventHub.$emit('goodCategoryLoaded');
