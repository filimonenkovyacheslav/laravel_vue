<template>
    <ul class="actions">
        ...
        <li class="favorite-btn" v-if="inArray('favorite-btn', items)" v-on:click="userRole ? toggleFavorite(entityData.id, 'news') : window.location.href = route('login')">
        ...
        <li class="edit-btn" v-if="userRole=='administrator'">
            <span class="edit_entity" title="Edit">
                <a v-bind:href="route('news.edit.admin', {'id': entityData.id})" target="_blank">
        ...
        <li class="edit-btn" v-if="userRole=='administrator'">
            <span class="delete_entity" title="Delete">
                <a v-bind:href="route('news.delete.admin', {'id': entityData.id})">
